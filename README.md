...
<p>Сервер в данный момент доступен через hamachi. Для подключения необходимо подключиться к существующей сети LensonNet. Сам сервер будет доступен по адресу http://25.31.176.32:8000 (как в hamachi организовать https еще догадались :))</p>
